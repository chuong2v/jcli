...
module.exports = function (issues) {
  this.get = function (issueKey, options) {
    if (issueKey) {
      try {
        var issueNotes;
        var filePath = path.resolve(homedir, `.${appName}/notes`, issueKey + ".json");
        issueNotes = require(filePath);

        debug(appName + ":notes")("issueNotes ", issueNotes);
        if (options.notesKey) {
    ...
    return fs.readdirSync(path.resolve(homedir, `.${appName}/notes`)).reduce(function (list, file) {
      if (mime.getType(file) === "application/json") {
        var issue = require(path.resolve(homedir, `.${appName}/notes`, file));
