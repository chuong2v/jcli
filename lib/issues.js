...
module.exports = function () {
  ...
  this.fetch = options => {
    ...
    debug(appName + ":issues")("qsStr: ", qsStr);

    return request(config.host + "/rest/api/2/search", {
      qs: queryObj,
      auth: config.auth
    }).then(result => {
      debug(appName + ":issues")("result: ", result);
      JSON.parse(result).issues.forEach(issue => {
        debug(appName + ":issues:issue")("[" + issue.key + "]" + issue.fields.summary);
        log("[" + issue.key + "]" + issue.fields.summary);
      });
    }).catch(error => {
      debug(appName + ":issues")("error: ", (error));
  ...
  this.get = (issueId, options) => {
    ...
    return request(config.host + "/rest/api/2/issue/" + issueId, {
      ...
      debug(appName + ":issue:result")("result: ", result);
      !(options.notShowLog) && log("result: ", result);
      return JSON.parse(result);
    }).catch(error => {
      debug(appName + ":issue:error")("error: ", (error));
  ...
  this.create = options => {
    ...
    return request({
      ...
      debug(appName + ":issue:create:result")("result: ", result);
      log("result: ", result);
    }).catch(error => {
      debug(appName + ":issue:create:error")("error: ", (error));
      log("error: ", error.toString());
    });
  };
