...
module.exports = function () {
  ...
  this.fetch = options => {
    ...
    debug("jcli:issues")("qsStr: ", qsStr);

    return request(config.host + "/rest/api/2/search", {
      qs: queryObj,
      auth: config.auth
    }).then(result => {
      debug("jcli:issues")("result: ", result);
      JSON.parse(result).issues.forEach(issue => {
        debug("jcli:issues:issue")("[" + issue.key + "]" + issue.fields.summary);
        log("[" + issue.key + "]" + issue.fields.summary);
      });
    }).catch(error => {
      debug("jcli:issues")("error: ", (error));
  ...
  this.get = (issueId, options) => {
    ...
    return request(config.host + "/rest/api/2/issue/" + issueId, {
      ...
      debug("jcli:issue:result")("result: ", result);
      !(options.jcli_not_show) && log("result: ", result);
      return JSON.parse(result);
    }).catch(error => {
      debug("jcli:issue:error")("error: ", (error));
  ...
  this.create = options => {
    ...
    return request({
      ...
      debug("jcli:issue:create:result")("result: ", result);
      log("result: ", result);
    }).catch(error => {
      debug("jcli:issue:create:error")("error: ", (error));
      log("error: ", error.toString());
    });
  };
