...
var configPath = process.env.NODE_CONFIG_PATH || path.resolve(homedir, `.${appName}/config.json`);
...
if (fs.existsSync(configPath)) {
  ...
  if (!fs.existsSync(path.resolve(homedir, `.${appName}`))) {
    fs.mkdirSync(path.resolve(homedir, `.${appName}`));
  }
  if (!fs.existsSync(path.resolve(homedir, `.${appName}/notes`))) {
    fs.mkdirSync(path.resolve(homedir, `.${appName}/notes`));
  }
